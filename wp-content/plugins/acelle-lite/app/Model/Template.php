...
class Template extends Model
{
        ...
        if (!empty($filters)) {
            if (!empty($filters['from'])) {
                if ($filters['from'] == 'mine') {
                    ...
                    $query = $query->where('shared', '=', true)->where('user_id', '!=', $user->id);
                } else {
                    $query = $query->where('shared', '=', true)->orWhere('user_id', '=', $user->id);
                }
            }
        }
    ...
    public static function tags($list=null)
    {
        ...
        $tags = array_merge($tags, [
            ['name' => 'UNSUBSCRIBE_URL', 'required' => (\Auth::user()->getOption('frontend', 'unsubscribe_url_required') == 'yes' ? true : false)],
